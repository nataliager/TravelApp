<template>
  <div class="create_flight">
    <div class="container_create_flight">
      <h2>Crear vuelo</h2>

      <form v-on:submit.prevent="processCreateFlight">
        <input
          type="text"
...
export default {
  ...
  methods: {
    processCreateFlight: function () {
      axios
        .post(
          /*manera asincronica*/
          "https://be-travel-app.herokuapp.com/flightCreate",
          this.flight,
          { headers: {} }
        )
        .then((result) => {
          let dataCreateFlight = {
            ...
            weight: this.flight.weight,
            price: this.flight.price,
          };

          alert("Se agrego el vuelo exitosamente!!");
        })
        .catch((error) => {
          console.log(error);
          alert("ERROR: Fallo en el registro del vuelo.");
        });
    },
  },
};
